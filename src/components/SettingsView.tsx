  ...
  return (
    <div className="min-h-dvh pb-20">
      ...
      <div className="px-4 mt-4 space-y-5">
        ...
        <section className="bg-card border rounded-xl p-4 space-y-3">
          <Label className="text-sm font-semibold">{t.aiRules}</Label>
          <p className="text-xs text-muted-foreground">{t.aiRulesDesc}</p>
          <Textarea
            value={settings.aiRules ?? ""}
            onChange={(e) => updateSettings({ ...settings, aiRules: e.target.value })}
            placeholder={t.aiRulesPh}
        ...
        <section className="bg-card border rounded-xl p-4 space-y-2">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed pt-2">{t.perDeviceNotice}</p>
